...
namespace PPJoy
{
    ...
    internal enum FileDevices : uint {
            VirtualDevice = (UInt32) NativeMethods.EFileDevice.Unknown, //identifies the PPJoy device type
            DeviceBus = (UInt32) NativeMethods.EFileDevice.BusExtender,
    }
    /// <summary>
     ...
     internal struct JoystickState 
    {
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = VirtualJoystick.MaxAnalogDataSources)]
        internal Int32[] Analog;                 // Analog input values
        internal Byte NumDigital;                // Number of digital values we pass
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = VirtualJoystick.MaxDigitalDataSources)]
        internal Byte[] Digital;                 // Digital input values
    } ;
    /// <summary>
    ...
    internal struct EnumerateDevicesMessage
    {
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = VirtualJoystick.MaxVirtualDevices)]
        internal DeviceInfo[] Joysticks;	    // Array of joystick records (holds return values)
    }
    ...
    internal struct JoystickMapPayload
    {
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = (int)Headers.MaxMappingPayloadLength)]
        internal Byte[] Data; //the raw map data
    }
    /// <summary>
    ...
    internal class Headers
    {
        ...
        internal const UInt32 MaxMappingPayloadLength = 250;//(uint)(4 * VirtualJoystick.MaxVisibleAxes) + (3 * VirtualJoystick.MaxVisibleButtons) + (6 * VirtualJoystick.MaxVisiblePovs);
        ...
        {
            return NativeMethods.GetIoCtlCode((uint)FileDevices.VirtualDevice, _index_, NativeMethods.EMethod.Buffered, NativeMethods.ECTL_CODEFileAccess.FileAnyAccess );
        }
        ...
        {
            return NativeMethods.GetIoCtlCode((uint)FileDevices.DeviceBus, _index_, NativeMethods.EMethod.Buffered, NativeMethods.ECTL_CODEFileAccess.FileAnyAccess);
        }
