...
namespace F4KeyFile
{
    ...
    public sealed class KeyFile
    {
        ...
        {
            foreach (IBinding thisBinding in this.Bindings)
            {
                if (thisBinding.Callback != null && callback !=null && thisBinding.Callback.ToLowerInvariant().Trim() == callback.ToLowerInvariant().Trim())
                {
                    return thisBinding;
                }
            }
        ...
        public IBinding[] Bindings
        {
            ...
            set
            {
                _bindings = new List<IBinding>(value);
            }
        }
        public void Load()
        {
