...
namespace Lzss
{
    public static class Codec
    {
        ...
        {
            int outputBufferSize=toCompress.Length > DEFAULT_OUTPUT_BUFFER_SIZE? toCompress.Length + 1024:DEFAULT_OUTPUT_BUFFER_SIZE;
            byte[] compressedDataBuffer = new byte[outputBufferSize];

            int compressedDataSize =Compress(toCompress, 0, toCompress.Length, compressedDataBuffer, 0);
            byte[] toReturn = new byte[compressedDataSize];
        ...
        {
            byte[] toReturn = new byte[uncompressedDataSize];
        ...
        {
            byte[] nonCompressedData = new byte[nonCompressedDataLength];
            nonCompressedDataStream.Read(nonCompressedData, 0, nonCompressedDataLength);
            byte[] compressedData = Compress(nonCompressedData);
        ...
        public static void Decompress(Stream compressedDataStream, int compressedDataLength, Stream outputStream, int uncompressedDataLength)
        {
            ...
            byte[] decompressedData = Decompress(compressedData, uncompressedDataLength);
        ...
        {
            GCHandle gchNonCompressedDataBuffer = GCHandle.Alloc(nonCompressedDataBuffer);
            GCHandle gchCompressedDataBuffer = GCHandle.Alloc(compressedDataBuffer);
            IntPtr nonCompressedDataBufferStartAddr = Marshal.UnsafeAddrOfPinnedArrayElement(nonCompressedDataBuffer, nonCompressedDataStartingOffset);
            IntPtr compressedDataBufferStartAddr = Marshal.UnsafeAddrOfPinnedArrayElement(compressedDataBuffer, compressedDataBufferStartingOffset);
            int compressedDataLength =NativeMethods.LZSS_Compress(nonCompressedDataBufferStartAddr, compressedDataBufferStartAddr, nonCompressedDataLength);
        ...
        {
            GCHandle gchDecompressedDataBuffer = GCHandle.Alloc(decompressedDataBuffer);
            GCHandle gchCompressedDataBuffer = GCHandle.Alloc(compressedDataBuffer);
            IntPtr decompressedDataBufferStartAddr = Marshal.UnsafeAddrOfPinnedArrayElement(decompressedDataBuffer, decompressedDataStartingOffset);
            IntPtr compressedDataBufferStartAddr = Marshal.UnsafeAddrOfPinnedArrayElement(compressedDataBuffer, compressedDataBufferStartingOffset);
            int bytesProcessed = NativeMethods.LZSS_Expand(compressedDataBufferStartAddr, decompressedDataBufferStartAddr, decompressedDataLength);
