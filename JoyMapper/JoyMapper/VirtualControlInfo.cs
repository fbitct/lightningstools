...
namespace JoyMapper
{
    ...
    public sealed class VirtualControlInfo
    {
        ...
        /// </summary>
        private ControlType _controlType = ControlType.Unknown;
        /// <summary>
        ...
        /// <param name="controlType">a value from the ControlType enumeration, specifying what type of control this object should represent (button, axis)</param>
        /// <param name="controlNum">a zero-based index identifying the "offset" of this control in
        /// the collection of similar virtual controls.  For example, Digital0 would be {controlType= ControlType.Button; controlNum=0}; Analog0 would be {controlType=ControlType.Axis; controlNum=0}, etc.
        /// and the first virtual axis </param>
        public VirtualControlInfo(VirtualDeviceInfo parent, ControlType controlType, int controlNum)
        {
            ...
            int PPJoyDeviceNumber = parent.VirtualDeviceNum;
            ...
            {
                throw new ArgumentOutOfRangeException("PPJoyDeviceNumber", "Must be in the range 1 - " + VirtualJoystick.MaxVirtualDevices);
            }
            else if (controlType == ControlType.Axis)
            {
                if (controlNum < 0 || controlNum >= VirtualJoystick.MaxAnalogDataSources)
                {
                    throw new ArgumentOutOfRangeException("controlNum", "Must be in the range 0 - (VirtualJoystick.MaxAnalogAxes-1) when controlType == ControlType.Axis");
                }
            }
            else if (controlType == ControlType.Button)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("controlNum", "Must be in the range 0 - (VirtualJoystick.MaxDigitalButtons-1) when controlType == ControlType.Button");
                }
            }
        ...
        #endregion
        #region Object Overrides (Equals, GetHashCode, ToString)
        ...
        {
            return ("Parent:{" + Parent.ToString() + "}:ControlType:" + ControlType.ToString() + ":ControlNum:" +  ControlNum.ToString());
        }
        #endregion
    }
}
