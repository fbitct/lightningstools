...
namespace JoyMapper
{
    internal sealed partial class frmMain : Form
    {
                    ...
                    if (deviceNode.Tag is PhysicalDeviceInfo)
                    {
                        ...
                        foreach (TreeNode controlCategoryNode in deviceNode.Nodes)
                        {
                            foreach (TreeNode controlNode in controlCategoryNode.Nodes)
                            {
                                try
                                {
                                    if (controlNode.Tag is PhysicalControlInfo)
                                    {
        ...
        /// <param name="overrideExistingMappings">If true, then existing 
        /// mappings in the target map will be overridden by mappings 
        /// from the source map.  If false, then mappings that already 
        /// exist in the target map will be preserved as-is.</param>
        private void MergeMaps(OutputMap sourceMap, OutputMap targetMap, bool overrideExistingMappings)
        {
            if (targetMap == null)
            {
            ...
            if (sourceMap == null)
            {
                throw new ArgumentNullException("sourceMap");
            }
            foreach (PhysicalControlInfo physicalControl in sourceMap.PhysicalControls)
            {
        ...
        /// returns NULL.
        /// </summary>
        /// <returns>a VirtualControlInfo object representing the next-available unmapped 
        ...
        /// returns NULL.
        /// </summary>
        /// <param name="physicalControl">A PhysicalControlInfo object representing the physical control 
        /// that will be mapped to the next-available corresponding virtual axis data source</param>
        /// <returns>a VirtualControlInfo object representing the next-available unmapped 
        /// analog data source across all virtual joysticks, or NULL if no available 
        /// unmapped corresponding virtual analog data source can be found.</returns>
        ...
        private bool CompareAxisTypes(AxisMapping virtualControl, PhysicalControlInfo physicalControl)
        {
            bool areSame = false;
        ...
        private void CreateNewOutputMap()
        {
            ...
            BIDeviceManager biDeviceManager = BIDeviceManager.GetInstance();
            ...
            if (biDevices != null)
            {
                ...
                foreach (BIPhysicalDeviceInfo biDevice in biDevices)
                {
                    ...
                    BIDeviceMonitor dev = BIDeviceMonitor.GetInstance(biDevice);

                    //add all the device's controls to the map, mapping them to nothing (null).
                    //This will ensure that they appear in the control tree on the form, without
                    //having any explicit mappings defined at this point in time.
                    foreach (PhysicalControlInfo pci in biDevice.Controls)
                    {
                        _outputMap.SetMapping(pci, null);
                    }
                }
            ...
            PHCCDeviceManager phccDeviceManager = PHCCDeviceManager.GetInstance();
        ...
        /// </summary>
        private void DisableSaveFunctionality()
        {
        ...
        /// </summary>
        /// <param name="inputControl">a PhysicalControlInfo object representing the 
        /// physical control whose mappings will be displayed</param>
        private void DisplayCurrentMapping(PhysicalControlInfo inputControl)
        {
            ...
            VirtualControlInfo outputControl = _outputMap.GetMapping(inputControl);
            ...
            if (outputControl == null)
            {
                ...
                int unmappedIndex = cboVirtualDevice.FindStringExact("<unmapped>");
        ...
        /// </summary>
        private void EnableSaveFunctionality()
        {
        ...
        /// </summary>
        /// <returns>false if the user cancelled the operation, and true if the user
        ...
        /// the return true condition herein.</returns>
        ...
        /// </summary>
        /// <returns>True if the operation requesting the prompt should continue, or false if the user cancelled the save operation.</returns>
        private bool PromptForSaveChanges(string promptText)
        {
            bool shouldContinue = false; //flag for holding the results of the user prompt to save changes
            ...
            if (_hasChanges) //if there are unsaved changes in the output map
            {
                ...
                else if (promptForSave == DialogResult.No)
                {
                ...
                else if (promptForSave == DialogResult.Yes)
                {
                    ...
                    {
                        //the user cancelled out of the save-as dialog box, so we have to abort
                        //the file-new operation as well, since they haven't had a chance to
        ...
        {
            bool shouldContinue = PromptForSaveChanges();
            if (!shouldContinue)
            {
                return false;
            }
        ...
        /// </summary>
        /// <param name="inputControl">a PhysicalControlInfo object representing the physical control whose possible mappings will be populated in the Mappings tab's controls</param>
        /// <param name="virtualDevice">a VirtualDeviceInfo object representing the currently-selected Virtual Device in the Mappings tab's virtual device selector combobox</param>
        ...
        /// </summary>
        /// <param name="pci">a PhysicalControlInfo object representing the input control whose descriptive text representation will be returned.</param>
        ...
        private string GetControlDisplayText(PhysicalControlInfo pci)
        {
            ...
            if (controlName != null)
            {
                if (pci.Alias != null && pci.Alias.Trim() != controlName.Trim())
                {
        ...
        private void RefreshDeviceList()
        {
            ...
            if (biDevices != null)
            {
                ...
                foreach (BIPhysicalDeviceInfo biDevice in biDevices)
                {
                    ...
                    BIDeviceMonitor monitor = BIDeviceMonitor.GetInstance(biDevice);
                    foreach (PhysicalControlInfo pci in biDevice.Controls)
                    {
                        //add the current control to the master list of controls
                        detectedInputControls.Add(pci);
            ...
            _mediator.ResetMonitors();
            PHCCDeviceManager phccManager = PHCCDeviceManager.GetInstance();
            PHCCPhysicalDeviceInfo[] phccDevices = null;
            try
            {
                phccDevices = phccManager.GetDevices();
            }
            catch (ApplicationException e)
            {
                _log.Debug(e.Message, e);
            ...
            foreach (PhysicalControlInfo pci in _outputMap.PhysicalControls)
            {
        ...
        private bool SaveMappingsFile(FileInfo file)
        {
            //store a reference to the currently-selected node so we can re-select it after saving
            TreeNode currentNode = treeMain.SelectedNode;
            ...
            //the save operation must have been successful if we've reached here, so
        ...
        private void SetMenuAndToolbarEnabledDisabledStates()
        {
            ...
            if (treeMain.SelectedNode != null) //if there's a selected node in the treeview
            {
                ...
                {
                    if (treeMain.SelectedNode.Tag != null && treeMain.SelectedNode.Tag is PhysicalDeviceInfo)
                    {
        ...
        /// </summary>
        /// <param name="inputControl">a PhysicalControlInfo object representing the physical control to which the mappings specified by the user will apply</param>
        private void StoreCurrentMapping(PhysicalControlInfo inputControl)
        {
            ...
            else //the user must have selected an actual, valid mapping
            {
                ...
                string virtualDeviceNumber = selectedVirtualDeviceName.Replace("PPJoy Virtual joystick ", "").Trim();
                int PPJoyDeviceNumber = Int32.Parse(virtualDeviceNumber);
                ...
                foreach (string subkeyName in subkeys)
                {
                        ...
                        subkeyName.StartsWith("PPJoy Joystick Driver", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        if (!string.IsNullOrEmpty(cplPath))
                        {
                            ...
                            {
                                return (Path.Combine(fi.Directory.FullName, "pportjoy.cpl"));
                            }
                        }
                        return null;
                    }
        ...
        /// </summary>
        /// <param name="node">a TreeNode from the main control tree, representing a physical control to check whether its output mappings are enabled.</param>
        /// <returns>a boolean indicating if the physical control represented by the supplied TreeNode has its output mappings enabled.</returns>
        private bool IsNodeEnabled(TreeNode node)
        {
            ...
            if (tag is PhysicalControlInfo) //if the tag represents a Physical Control
            {
                ...
                {
                    return false; //mapping is NOT enabled for this physical control
                }
            }
            else if (tag is PhysicalDeviceInfo) //if the tag represents a Phyaical Device
            {
                ...
                {
                    return false; //mapping is NOT enabled for any of this device's controls
                }
            }
            else //this node is a container node and is not a Device container node, so we'll have to
                //recursively call the IsNodeEnabled() function for each of the children
                //of this node, and if any of them are enabled, then we can say that this container
                //node is enabled as well.
            {
        ...
        /// </summary>
        /// <param name="control">a PhysicalControlInfo object representing the physical input control whose TreeView node will be returned.</param>
        /// <returns>a TreeNode object representing the node in the TeeView that is associated with the specified physical input control.</returns>
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (set.LoadDefaultMappingFile)
            {
                ...
                {
                    if (set.StartMappingOnLaunch) //if we're supposed to start mapping immediately on launch
                    {
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">Event arguments for the Click event.</param>
        private void mnuPPJoyOpenPPJoyControlPanel_Click(object sender, EventArgs e)
        {
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">Event arguments for the Click event.</param>
        private void mnuToolsOpenWindowsJoystickControlPanel_Click(object sender, EventArgs e)
        {
        ...
        private void mnuPPJoy_DropDownOpening(object sender, EventArgs e)
        {
            ...
            {
                mnuPPJoyCreateNewVirtualDevices.Enabled = true;
            }
        ...
        private void mnuFileImport_Click(object sender, EventArgs e)
        {
            ...
            OutputMap currentDevicesAndControls = _outputMap;
