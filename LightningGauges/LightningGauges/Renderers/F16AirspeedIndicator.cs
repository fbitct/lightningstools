...
namespace LightningGauges.Renderers
{
    public class F16AirspeedIndicator : InstrumentRendererBase, IDisposable
    {
        ...
        public F16AirspeedIndicator()
            : base()
        {
            this.InstrumentState = new F16AirspeedIndicatorInstrumentState();
        }
        ...
        private void LoadImageResources()
        {
            if (_background == null)
            {
                _background = (Bitmap)Common.Imaging.Util.LoadBitmapFromFile(IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_BACKGROUND_IMAGE_FILENAME);
            }
            if (_machWheel == null)
            {
                _machWheel = ImageMaskPair.CreateFromFiles(
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_MACH_WHEEL_IMAGE_FILENAME,
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_MACH_WHEEL_MASK_FILENAME
                    );
            ...
            if (_speedBug == null)
            {
                _speedBug = ImageMaskPair.CreateFromFiles(
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_SPEED_BUG_IMAGE_FILENAME,
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_SPEED_BUG_MASK_FILENAME
                    );
            }
            ...
            if (_airspeedPointerWheel == null)
            {
                _airspeedPointerWheel = ImageMaskPair.CreateFromFiles(
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_POINTER_IMAGE_FILENAME,
                    IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + AIRSPEED_POINTER_MASK_FILENAME
                    );
        ...
        public override void Render(Graphics g, Rectangle bounds)
        {
            ...
            lock (_imagesLock)
            {
                ...
                float airspeed = this.InstrumentState.AirspeedKnots;
                float airspeedIndex = this.InstrumentState.AirspeedIndexKnots;
                float mach = this.InstrumentState.MachNumber;
                ...
                airspeedAngle = (airspeedAngle + 128.0f) % 360.0f; //compensate for pointer image not being oriented with zero up
                ...
                g.TranslateTransform(-125.5F, -127.5F);
                g.DrawImage(_machWheel.MaskedImage, new Rectangle(0, 0, _machWheel.MaskedImage.Width, _machWheel.MaskedImage.Height), new Rectangle(0, 0, _machWheel.MaskedImage.Width, _machWheel.MaskedImage.Height), GraphicsUnit.Pixel);
                GraphicsUtil.RestoreGraphicsState(g, ref basicState);
                ...
                g.TranslateTransform(-126F, -127F);
                g.DrawImage(_airspeedPointerWheel.MaskedImage, new Rectangle(0, 0, _airspeedPointerWheel.MaskedImage.Width, _airspeedPointerWheel.MaskedImage.Height), new Rectangle(0, 0, _airspeedPointerWheel.MaskedImage.Width, _airspeedPointerWheel.MaskedImage.Height), GraphicsUnit.Pixel);
                GraphicsUtil.RestoreGraphicsState(g, ref basicState);
                ...
                g.TranslateTransform(-126, -130);
                g.DrawImage(_speedBug.MaskedImage, new Rectangle(0, 0, _speedBug.MaskedImage.Width, _speedBug.MaskedImage.Height), new Rectangle(0, 0, _speedBug.MaskedImage.Width, _speedBug.MaskedImage.Height), GraphicsUnit.Pixel);
                GraphicsUtil.RestoreGraphicsState(g, ref basicState);
        ...
        private float GetPointerAngle(float airspeed)
        {
            float angle;
        ...
        public F16AirspeedIndicatorInstrumentState InstrumentState
        {
            get;
            set;
        }
        ...
        [Serializable]
        public class F16AirspeedIndicatorInstrumentState : InstrumentStateBase
        {
            ...
            public F16AirspeedIndicatorInstrumentState():base()
            {
                this.AirspeedIndexKnots = DEFAULT_AIRSPEED_INDEX_KNOTS;
