...
namespace LightningGauges.Renderers
{
    public class F16FuelQuantityIndicator : InstrumentRendererBase, IDisposable
    {
        ...
        public F16FuelQuantityIndicator()
            : base()
        {
            this.InstrumentState = new F16FuelQuantityIndicatorInstrumentState();
            this.Options = new F16FuelQuantityIndicatorOptions();
        }
        ...
        private void LoadImageResources()
        {
            ...
            if (_digits == null)
            {
                _digits = (Bitmap)Common.Imaging.Util.LoadBitmapFromFile(IMAGES_FOLDER_NAME + Path.DirectorySeparatorChar + FUEL_DIGITS_FILENAME);
        ...
        public override void Render(Graphics g, Rectangle bounds)
        {
            ...
            lock (_imagesLock)
            {
                ...
                float hundredsDigit = (this.InstrumentState.TotalFuelQuantityPounds / 100.0f) % 10.0f;
                float thousandsDigit = (float)Math.Truncate((this.InstrumentState.TotalFuelQuantityPounds / 1000.0f)) % 10.0f;
                float tenThousandsDigit = (float)Math.Truncate((this.InstrumentState.TotalFuelQuantityPounds / 10000.0f) % 10.0f);
                ...
                float aftLeftNeedleAngle = baseAngle + GetAngle(this.InstrumentState.AftLeftFuelQuantityPounds);
                GraphicsUtil.RestoreGraphicsState(g, ref basicState);
                ...
                g.TranslateTransform(-_background.MaskedImage.Width / 2.0f, -_background.MaskedImage.Height / 2.0f);
                if (this.Options.NeedleType == F16FuelQuantityIndicatorOptions.F16FuelQuantityNeedleType.CModel)
                {
                    g.DrawImage(_aftLeftArrowCModel.MaskedImage, new Point(0, 0));
                }
                else if (this.Options.NeedleType == F16FuelQuantityIndicatorOptions.F16FuelQuantityNeedleType.DModel)
                {
                ...
                float foreRightNeedleAngle = baseAngle + GetAngle(this.InstrumentState.ForeRightFuelQuantityPounds);
                GraphicsUtil.RestoreGraphicsState(g, ref basicState);
                g.TranslateTransform(_background.MaskedImage.Width / 2.0f, _background.MaskedImage.Height / 2.0f);
        ...
        public F16FuelQuantityIndicatorInstrumentState InstrumentState
        {
            get;
            set;
        }
        public F16FuelQuantityIndicatorOptions Options
        {
            get;
            set;
        }
        ...
        public class F16FuelQuantityIndicatorInstrumentState : InstrumentStateBase
        {
            ...
            private float _foreRightFuelQuantityPounds = 0;
            private float _aftLeftFuelQuantityPounds = 0;
            private float _totalFuelQuantityPounds = 0;

            public F16FuelQuantityIndicatorInstrumentState():base()
            {
            }
            public float ForeRightFuelQuantityPounds
            {
                get
                {
                    return _foreRightFuelQuantityPounds;
                }
                set
                {
                    float qty = value;
            ...
            public float AftLeftFuelQuantityPounds
            {
                get
                {
                    return _aftLeftFuelQuantityPounds;
                }
                set
                {
                    float qty = value;
            ...
            public float TotalFuelQuantityPounds
            {
                get
                {
                    return _totalFuelQuantityPounds;
                }
                set
                {
                    float qty = value;
        ...
        public class F16FuelQuantityIndicatorOptions
        {
            ...
            public F16FuelQuantityIndicatorOptions():base()
            {
            }
            public F16FuelQuantityNeedleType NeedleType
            {
                get;
                set;
            }
