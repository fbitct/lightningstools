...
namespace LightningGauges.Renderers.F16.ISIS
{
    ...
    {
        internal static void DrawClimbPitchBars(Graphics g, float pitchDegrees, int minorLineWidth, float pitchLadderWidth,
            ...
            StringFormat pitchDigitFormat)
        {
            ...
            for (float i = -85; i <= 85; i += 2.5f)
            {
                ...
                //draw this line
                g.DrawLineFast(pitchBarPen, new PointF((pitchLadderWidth/2.0f) - (lineWidth/2.0f), y),
                    new PointF((pitchLadderWidth/2.0f) + (lineWidth/2.0f), y));

                //draw the pitch digits
                if (i%10 == 0)
                {
                    PitchDigitRenderer.DrawPitchDigits(g, pitchLadderWidth, pitchDigitFont, pitchDigitBrush, pitchDigitFormat, i, lineWidth, y);
                }
            }
        }
    }
