...
namespace LightningGauges.Renderers.F16.AzimuthIndicator
{
    ...
    {
        internal static void DrawBlip(Graphics gfx, ref GraphicsState basicState, Matrix initialTransform,
            float atdRingOffsetTranslateX, float atdRingOffsetTranslateY, float atdRingScale, int backgroundWidth,
            int backgroundHeight, Color missileColor, float outerRingTop, float innerRingTop, Color scopeGreenColor, Color airborneThreatColor,
            Color groundThreatColor, Color searchThreatColor, Color navalThreatColor, Color unknownThreatColor, Blip blip, InstrumentState instrumentState, Options options, Font missileWarningFont,
            int width, Font largeFont, Font smallFont)
        {
            ...
            MissileLaunchLineRenderer.DrawMissileLaunchLine(gfx, outerRingTop, innerRingTop, center, blip, launchLinePen, angle,
                missileWarningBrush, options, missileWarningFont);
            ...
            var emitterSymbolDestinationRectangle =
                new RectangleF((int) ((backgroundWidth - width)/2.0f),
                    (int)((backgroundHeight - width) / 2.0f), width, width);
            EmitterSymbolRenderer.DrawEmitterSymbol(blip.SymbolID, gfx, emitterSymbolDestinationRectangle, useLargeSymbol,
                usePrimarySymbol, emitterColor, largeFont, smallFont, width);

            var emitterPen = new Pen(emitterColor);
            gfx.TranslateTransform(-width / 2.0f,
                -width / 2.0f);
            if (blip.Selected > 0)
            {
                SelectedThreatDiamondRenderer.DrawSelectedThreatDiamond(gfx, backgroundWidth, backgroundHeight, emitterPen, width);
            }
            if ((blip.MissileActivity > 0 && blip.MissileLaunch == 0))
            {
                MissileActivitySymbolRenderer.DrawMissileActivitySymbol(gfx, backgroundWidth, backgroundHeight, emitterPen, width);
            }
            else if ((blip.MissileActivity > 0 && blip.MissileLaunch > 0))
            {
                MissileLaunchSymbolRenderer.DrawMissileLaunchSymbol(gfx, backgroundWidth, backgroundHeight, emitterPen, width);
            }
